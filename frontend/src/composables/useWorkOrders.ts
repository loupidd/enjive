...
export function useWorkOrders() {
  ...
  async function submitReport(id: string, data: {
    summary: string; workDone: string; hoursSpent: number;
    findings?: string; recommendations?: string; attachments?: string[];
  }) {
    saving.value = true; error.value = null;
    try {
  ...
  return { items, item, meta, loading, saving, error,
    fetch, fetchOne, create, update, updateStatus, submitReport, remove };
}
