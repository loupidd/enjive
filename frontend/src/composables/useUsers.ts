...
export function useUsers() {
  ...
  async function create(data: { email: string; password: string; firstName: string; lastName: string; role: string; phone?: string }) {
  ...
  async function changePassword(id: string, currentPassword: string, newPassword: string) {
    saving.value = true; error.value = null;
    try {
      await api.patch(`/users/${id}/password`, { currentPassword, newPassword });
    } catch (e) { error.value = extractError(e); throw e; }
    finally { saving.value = false; }
  }
  ...
  return { items, item, meta, loading, saving, error,
    fetch, fetchOne, create, update, changePassword, remove };
}
