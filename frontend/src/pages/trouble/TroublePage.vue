    ...
    <div class="card p-0 overflow-hidden">
                ...
                <div v-if="t.woStatus" class="flex items-center gap-1.5 mt-1">
                  <div class="h-1 w-16 bg-white/5 rounded-full overflow-hidden">
                    <div
                      ...
                      :class="woProgressColor(t.woStatus)"
                      :style="{ width: woProgressPct(t.woStatus) + '%' }"
                    />
                  </div>
                  <span
                    class="text-[10px]"
                    :class="woProgressColor(t.woStatus).replace('bg-', 'text-')"
                    >{{ t.woStatus }}</span
                  >
                </div>
                <span v-if="!t.woId" class="text-[10px] text-denim-200/25"
                  >No WO yet</span
                >
              </td>
              ...
              <td class="px-3 py-3">
                <div class="flex items-center justify-center gap-1">
                  ...
                  <button
                    ...
                    title="Schedule Corrective WO"
                    @click="openScheduleWO(t)"
                  >
                    <svg
                      width="12"
                      height="12"
      ...
      >
        <p class="text-xs text-denim-200/40">
          Showing {{ displayedTroubles.length }} of
          {{ filteredTroubles.length }}
        </p>
        <div class="flex gap-1">
          <button
            class="px-3 py-1 text-xs rounded-lg hover:bg-denim-700/40 text-denim-200/50 disabled:opacity-30 transition-colors"
            :disabled="page === 1"
            @click="page--"
          >
            ← Prev
            ...
            <div class="px-6 py-5 space-y-4">
              ...
              <div class="grid grid-cols-2 gap-3">
                <div>
                  ...
                  >
                  <select
                    v-model="createForm.equipmentType"
                    class="input"
                    @change="createForm.equipmentId = ''"
                  >
                ...
                <div>
                  ...
                  <select
                    ...
                    class="input"
                    :disabled="!createForm.equipmentType"
                  >
              ...
              <div class="grid grid-cols-2 gap-3">
                <div>
                  <label class="label"
                    >Operation Status <span class="text-red-400">*</span></label
                  >
              ...
              <div class="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label class="label"
                    >Hours <span class="text-red-400">*</span></label
                  >
                  <input
              ...
              <div>
                ...
                <textarea
                  ...
                  rows="2"
                  placeholder="Describe the equipment's current condition..."
                />
              </div>
              ...
              <div>
                <label class="label"
                  >Impact <span class="text-red-400">*</span></label
                >
                <textarea
                  v-model="createForm.impact"
                  class="input resize-none"
                  rows="2"
                  placeholder="What is the operational impact?"
                />
              </div>
              ...
              <div>
                <label class="label"
                  >Early Action Taken <span class="text-red-400">*</span></label
                >
                <textarea
              ...
              <div>
                <label class="label"
                  >Chronology <span class="text-red-400">*</span></label
                >
                <textarea
                  v-model="createForm.chronology"
                  class="input resize-none"
                  rows="3"
                  placeholder="Describe the sequence of events leading to this trouble..."
                />
              </div>
              ...
              <div>
                ...
                >
                  <svg
                    ...
                    stroke="currentColor"
                    stroke-width="1.5"
                    class="mx-auto mb-2 text-denim-200/30 group-hover:text-caramel/50 transition-colors"
                  >
                  ...
                  <p
                    class="text-xs text-denim-200/40 group-hover:text-denim-200/60"
                  >
                    Click to upload images
                  </p>
                  <p class="text-[10px] text-denim-200/25 mt-1">
                    Multiple images supported · Each with a note
                  </p>
                </div>
                <!-- Attachment previews -->
                <div
          ...
          >
            <div class="px-6 py-5 border-b border-denim-700/40">
              ...
              <p class="text-xs font-mono text-denim-200/50 mt-1">
                {{ ackTrouble?.id }} · {{ ackTrouble?.equipmentId }}
              </p>
            </div>
            <div class="px-6 py-4 space-y-4">
              ...
              <div class="bg-denim-900/50 rounded-xl p-4 space-y-2 text-xs">
                ...
                <div class="flex justify-between">
                  <span class="text-denim-200/50">Operation</span
                  ><span
                    :class="opColor(ackTrouble?.operation)"
                    class="font-semibold px-2 py-0.5 rounded-full text-[10px]"
                    >{{ ackTrouble?.operation }}</span
                  >
                </div>
                <div class="flex justify-between">
                ...
                <div class="flex justify-between">
                  ...
                  ><span
                    class="font-bold"
                    :class="
                      ackTrouble?.age > 7 ? 'text-red-400' : 'text-yellow-300'
                    "
    ...
    <Teleport to="body">
      <Transition name="modal">
        ...
        >
          <div class="absolute inset-0 bg-black/70 backdrop-blur-sm" />
          <div
            class="relative bg-denim-800 border border-caramel/15 rounded-2xl shadow-2xl w-full max-w-sm"
          >
            ...
            <div class="px-6 py-4 space-y-3">
              <div class="bg-denim-900/50 rounded-lg p-3 text-xs space-y-1">
                <div class="flex justify-between">
                  ...
                  ><span class="text-white font-mono">{{
                    scheduleWOTrouble?.equipmentId
                  }}</span>
                </div>
                <div class="flex justify-between">
                  <span class="text-denim-200/40">Trouble</span
                  ><span class="text-white">{{ scheduleWOTrouble?.name }}</span>
                </div>
              </div>
              <div>
                <label class="label"
                  >Scheduled Date <span class="text-red-400">*</span></label
                >
                <input v-model="woDate" type="date" class="input" />
              </div>
              <div>
                <label class="label"
                  >Assign Technician <span class="text-red-400">*</span></label
                >
              ...
              >
                💡 This will create a Corrective Work Order (interval: Once) and
                assign it to the Task list.
              </div>
            </div>
            <div class="px-6 pb-5 flex justify-end gap-2">
              <button class="btn-secondary" @click="showScheduleWO = false">
                Cancel
              </button>
              <button
                class="btn-primary"
                @click="submitScheduleWO"
                :disabled="!woDate || !woTechnician"
              >
                Create WO
              </button>
            </div>
          ...
          >
            ...
            <div class="px-6 py-4">
              <div class="relative pl-5">
                ...
                />
                <div
                  v-for="(event, i) in historyTrouble?.timeline"
                  :key="i"
...
const EQ_MAP: Record<string, { id: string; name: string }[]> = {
  "AC Fasilitas": [
    { id: "EDA_AC_001", name: "AC Lobby" },
    { id: "EDA_AC_002", name: "AC Function Room" },
  ],
  Genset: [
    { id: "EDA_GEN_001", name: "Genset B2" },
    { id: "EDA_GEN_002", name: "Genset Rooftop" },
  ],
  "Panel Listrik": [
    { id: "EDA_PNL_001", name: "Panel MDP B2" },
    { id: "EDA_PNL_002", name: "Panel Lt.3" },
  ],
  CCTV: [{ id: "EDA_CCTV_001", name: "CCTV Lobby" }],
  "Fire Alarm": [{ id: "EDA_FA_001", name: "FA Panel Utama" }],
  "Pompa Air": [
    { id: "EDA_PUMP_001", name: "Pompa Air Bersih" },
    { id: "EDA_PUMP_003", name: "Pompa Transfer" },
  ],
  "Lift / Elevator": [
    { id: "EDA_LIFT_001", name: "Lift Servis B1" },
    { id: "EDA_LIFT_002", name: "Lift Penumpang 1" },
  ],
};
    ...
    timeline: [
      {
        label: "Reported",
        color: "bg-red-500",
        date: "2026-02-25 08:12",
        by: "Ahmad Fauzi",
        note: "Genset fail to start on routine check",
      },
    ],
  },
  {
    ...
    timeline: [
      ...
      },
      {
        ...
        by: "Budi SPV",
        note: "Approved, schedule corrective WO",
      },
      {
        label: "WO Scheduled",
        color: "bg-blue-400",
        ...
        by: "Budi SPV",
        note: "WO-2026-010 created for 2026-03-05",
      },
    ],
  },
  {
    id: "TBL-2026-003",
    equipmentId: "EDA_PUMP_001",
    equipmentType: "Pompa Air",
    name: "Pompa air bocor di sambungan",
    date: "2026-03-01",
    age: 3,
    operation: "Operations",
    reporter: "Dodi Prasetyo",
    status: "Open-WO-Done",
    woId: "WO-2026-008",
    woStatus: "Finish",
    timeline: [
      ...
      },
      {
        ...
        by: "Ahmad Fauzi",
        note: "Leak fixed, clamp replaced",
      },
    ],
  },
  {
    ...
    name: "Pintu lift tidak menutup sempurna",
    date: "2026-02-10",
    age: 22,
    operation: "Down",
    ...
    timeline: [
      {
        label: "Reported",
        color: "bg-red-500",
        date: "2026-03-03 13:45",
        by: "Eko Wahyudi",
        note: "MCB trips 3x in 2 hours",
      },
    ],
  },
]);
...
const kpis = computed(() => [
  {
    ...
    ring: "border border-red-500/15 hover:border-red-500/30",
  ...
  },
  {
    ...
    ring: "border border-green-500/15 hover:border-green-500/30",
  ...
  },
  {
    ...
    ring: "border border-denim-600/20 hover:border-denim-600/40",
...
function ackLabel(t: any) {
  return t.status === "Alert" ? "Approve" : "ACK";
}
function ackStyle(t: any) {
  if (t.status === "Alert")
    return "bg-red-500/20 text-red-300 hover:bg-red-500/30 border border-red-500/30 shadow-sm shadow-red-500/10";
...
const ackTitle = computed(() => {
  ...
  if (ackTrouble.value.status === "Alert")
    return "SPV — Approve Trouble Report";
  return "Acknowledge";
});

function openAckModal(t: any) {
  ackTrouble.value = t;
  ackNote.value = "";
  showAck.value = true;
}
...
function openScheduleWO(t: any) {
  scheduleWOTrouble.value = t;
  woDate.value = "";
  woTechnician.value = "";
...
function submitScheduleWO() {
  ...
  const idx = troubles.value.findIndex(
    (t) => t.id === scheduleWOTrouble.value.id,
  );
  ...
  if (idx >= 0) {
    ...
    troubles.value[idx].timeline.push({
      label: "Corrective WO Scheduled",
      color: "bg-blue-400",
      date: new Date().toLocaleString(),
      by: "SPV",
      note: `${newWoId} scheduled for ${woDate.value} → ${woTechnician.value}`,
    });
  }
...
const filteredEquipment = computed(
  () => EQ_MAP[createForm.value.equipmentType] ?? [],
);
const createFormValid = computed(
  () =>
    createForm.value.equipmentType &&
    createForm.value.equipmentId &&
    createForm.value.operation &&
    createForm.value.name &&
    createForm.value.date &&
    createForm.value.hours &&
    ...
    timeline: [
      {
        ...
        date: new Date().toLocaleString(),
        by: "Current User",
        note: createForm.value.chronology,
      },
    ],
  });
