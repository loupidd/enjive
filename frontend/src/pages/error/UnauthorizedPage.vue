<template>
  <div
    class="min-h-screen bg-denim-950 flex items-center justify-center p-6 relative overflow-hidden"
  >
    <!-- Background grain -->
    ...
    <div
      ...
      :style="{
        background: isLoggedIn ? '#ef4444' : '#FFC677',
        top: '20%',
        left: '50%',
    ...
    <div class="relative z-10 text-center max-w-md w-full">
      <!-- Icon -->
      <div
        class="mx-auto w-20 h-20 rounded-2xl flex items-center justify-center mb-6 shadow-2xl"
        :class="
          isLoggedIn
            ? 'bg-red-500/15 border border-red-500/30'
            : 'bg-caramel/10 border border-caramel/30'
        "
      >
        <svg
          v-if="isLoggedIn"
      ...
      <div class="mt-10 flex items-center justify-center gap-2 opacity-30">
        ...
        <span class="text-xs text-denim-200/50 font-semibold tracking-wide"
          >EnJive CMMS</span
        >
      </div>
    </div>
