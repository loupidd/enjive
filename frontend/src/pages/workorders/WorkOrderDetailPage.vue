  ...
  <div class="space-y-4">
      ...
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-xs text-denim-200/50 mt-0.5">
            {{ wo.equipmentId }} · {{ wo.interval }} · Started
            {{ wo.startDate }}
          </p>
        </div>
      </div>
        ...
        <template v-for="(stage, i) in PIPELINE" :key="stage.key">
          <div class="flex flex-col items-center px-3 min-w-[72px]">
            ...
            <span
              ...
              class="text-[8px] text-denim-200/40 mt-0.5"
              >Now</span
            >
          </div>
          <div
            v-if="i < PIPELINE.length - 1"
            class="h-px flex-1 min-w-[16px] transition-colors"
            :class="
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
      ...
      <div class="space-y-4">
        ...
        <div class="card p-0 overflow-hidden">
          ...
          >
            ...
            <h3
              class="text-xs font-semibold text-denim-200/70 uppercase tracking-wide"
            >
              Work Order Info
            </h3>
          </div>
          <div class="divide-y divide-denim-700/20">
            ...
            >
              <span class="text-xs text-denim-200/40 w-28 shrink-0">{{
                row.label
              }}</span>
        ...
        <div class="card p-0 overflow-hidden">
          ...
          >
            ...
            <RouterLink
              ...
              class="text-[10px] text-caramel hover:underline flex items-center gap-1"
            >
          ...
          <div class="px-4 py-3 space-y-1.5 text-xs">
            <div class="flex justify-between">
              <span class="text-denim-200/40">ID</span
              ><span class="font-mono text-caramel font-semibold">{{
                wo.equipmentId
              }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-denim-200/40">Name</span
              ><span class="text-white">{{ wo.equipmentName }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-denim-200/40">Type</span
              ><span class="text-white">{{ wo.equipmentType }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-denim-200/40">Section</span
              ><span class="text-white">{{ wo.section }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-denim-200/40">Location</span
        ...
        >
          <div
            class="px-4 py-3 border-b border-orange-500/15 flex items-center gap-2"
          >
            <IconAlertTriangle :size="14" class="text-orange-400" />
            <h3
              class="text-xs font-semibold text-orange-300/70 uppercase tracking-wide"
            >
              Linked Trouble Report
            </h3>
          </div>
          <div class="px-4 py-3 space-y-1.5 text-xs">
            <div class="flex justify-between">
              <span class="text-denim-200/40">Trouble ID</span
            ...
            <div class="flex justify-between">
              <span class="text-denim-200/40">Operation</span
              ><span class="text-white">{{ wo.troubleOperation }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="lg:col-span-2 space-y-4">
        ...
        <div class="card p-0 overflow-hidden">
          ...
          >
            ...
            <span class="text-xs text-denim-200/40"
              >{{ completedCount }}/{{ wo.activities.length }} done</span
            >
          </div>
          <!-- Progress bar -->
          ...
          <div class="divide-y divide-denim-700/15">
            ...
            >
              <!-- Number -->
              <span
                class="text-[10px] text-denim-200/30 w-4 text-right shrink-0 mt-0.5"
                >{{ i + 1 }}</span
              >
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="flex items-center gap-2 mt-0.5 flex-wrap">
                  <span
                    ...
                    >{{ act.type }}</span
                  >
                  <span class="text-[10px] text-denim-200/40">{{
                    act.answerType
                  }}</span>
                  <span v-if="act.unit" class="text-[10px] text-denim-200/40"
                    >· {{ act.unit }}</span
                  >
                </div>
              </div>
              ...
              <div
                v-if="
                  (wo.status === 'Process' || wo.status === 'Waiting') &&
                  !act.done
                "
                class="shrink-0"
              >
                ...
                />
                <select
                  v-else-if="act.answerType === 'Qualitative'"
                  v-model="act.answer"
              ...
              <!-- Answered value (done) -->
              <div
                v-else-if="act.done && act.answer"
                class="text-xs font-bold text-caramel shrink-0"
              >
                {{ act.answer }}
                <span class="text-denim-200/40 font-normal">{{
                  act.unit
                }}</span>
              </div>
            </div>
        ...
        <div
          v-if="wo.status === 'Process' || wo.workReport"
          ...
          <div class="px-4 py-4 space-y-3">
            <div>
              ...
              <textarea
                ...
                placeholder="Describe equipment condition after maintenance..."
              />
            </div>
            <div>
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  v-model="workReportForm.spareParts"
                  class="input text-xs"
                  :disabled="!!wo.workReport"
                  placeholder="e.g. Filter, Freon R32"
                />
              </div>
              <div>
                <label class="label">Next Action</label>
                <input
                  v-model="workReportForm.nextAction"
                  class="input text-xs"
                  :disabled="!!wo.workReport"
                  placeholder="e.g. Monitor next week"
                />
              </div>
            </div>
            ...
            <!-- Trouble toggle -->
            <div
              class="flex items-center gap-3 py-1 border-t border-denim-700/20 pt-3"
            >
              <button
              ...
              <div>
                ...
                <p class="text-xs text-denim-200/40">
                  This will create a Trouble Report linked to this WO
                </p>
              </div>
              <span
                v-if="workReportForm.hasTrouble"
                class="ml-auto text-xs text-red-400 flex items-center gap-1"
              >
            ...
            <div v-if="!wo.workReport" class="flex justify-end gap-2 pt-1">
              <button
                class="btn-primary gap-1.5 text-xs"
                :disabled="completedCount < wo.activities.length"
                @click="submitReport"
              >
            ...
            <p
              v-if="completedCount < wo.activities.length && !wo.workReport"
              class="text-[10px] text-denim-200/40"
            >
              Complete all {{ wo.activities.length }} activities before
              submitting.
            </p>
          </div>
        </div>
        ...
        <div class="card p-0 overflow-hidden">
          ...
          <div class="px-4 py-4">
            <div class="relative pl-5">
              <div
                class="absolute left-1.5 top-2 bottom-2 w-px bg-denim-600/30"
              />
              <div
              ...
              >
                ...
                <p
                  v-if="ev.note"
                  class="text-[10px] text-denim-200/35 mt-0.5 italic bg-denim-700/20 rounded px-2 py-1"
                >
                  {{ ev.note }}
                </p>
              </div>
              <div
                v-if="!wo.timeline.length"
                class="text-xs text-denim-200/30 py-2"
              >
                No events yet
              </div>
            </div>
...
const woInfoRows = computed(() => [
  { label: "WO ID", value: wo.value.id, mono: true, badge: null },
  {
    ...
    value: wo.value.type,
    mono: false,
    badge: typeColorRaw(wo.value.type),
  },
  ...
  { label: "Technician", value: wo.value.technician, mono: false, badge: null },
  {
    ...
    mono: false,
    badge: statusBadge(wo.value.status),
  },
...
function typeColorRaw(t: string) {
  ...
  if (t === "Thermography Investigation")
    return "bg-purple-500/15 text-purple-300";
  return "bg-denim-600/40 text-denim-200";
}
function typeColor(t: string) {
  return typeColorRaw(t);
}
