<template>
  <div class="space-y-4">
    ...
    <div class="card p-0 overflow-hidden">
      <div
        class="flex items-center justify-between px-5 py-3 border-b border-denim-700/40"
      >
        <button class="btn-secondary px-3 py-1.5 text-xs" @click="prevMonth">
          ← Prev
        </button>
        <div class="flex items-center gap-4">
          ...
          <div class="hidden sm:flex items-center gap-3">
            <div
              v-for="l in LEGEND"
              :key="l.label"
              class="flex items-center gap-1.5 text-[10px] text-denim-200/50"
            >
        ...
        <button class="btn-secondary px-3 py-1.5 text-xs" @click="nextMonth">
          Next →
        </button>
      </div>
      ...
      <div class="grid grid-cols-7 border-b border-denim-700/40">
        <div
          v-for="d in DAYS"
          :key="d"
          class="py-2 text-center text-[11px] font-semibold text-denim-200/40 uppercase tracking-wide"
        >
          {{ d }}
        </div>
      </div>
      ...
      <div class="grid grid-cols-7">
        <div
          ...
          :class="[
            cell.currentMonth
              ? 'hover:bg-denim-700/15'
              : 'bg-denim-900/30 opacity-60',
            cell.isToday ? 'ring-1 ring-inset ring-caramel/40' : '',
          ]"
          @click="openCreate(cell.date)"
        >
          <span
            class="text-[11px] font-semibold w-6 h-6 flex items-center justify-center rounded-full mb-1"
            :class="
              cell.isToday
          ...
          <div class="space-y-0.5">
            ...
            >
              ...
              <span
                ...
                :class="woStatusDot(ev.woStatus)"
                :title="ev.woStatus"
              />
            </div>
            <div
              v-if="cell.events.length > 3"
              class="text-[9px] text-denim-200/35 pl-1.5"
            >
    ...
    <Teleport to="body">
      <Transition name="modal">
        ...
        >
          ...
          <div
            class="relative bg-denim-800 border border-denim-600/50 rounded-2xl shadow-2xl w-full max-w-lg max-h-[82vh] overflow-y-auto"
          >
            <div
            ...
            <div class="px-6 py-5 space-y-4">
              <div>
                ...
                <select
                  ...
                  class="input"
                  @change="form.equipmentId = ''"
                >
                  <option value="">Select type</option>
                  <option v-for="t in equipmentTypes" :key="t" :value="t">
                    {{ t }}
                  </option>
                </select>
              </div>
              ...
              <div class="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label class="label"
                    >Interval <span class="text-red-400">*</span></label
                  >
              ...
              <div class="flex items-center gap-3">
                ...
                <label class="text-sm text-slate-300"
                  >Repeat to next schedules</label
                >
              </div>
              <div
                v-if="duplicateWarning"
                class="flex items-start gap-2 bg-yellow-400/10 border border-yellow-400/20 rounded-lg px-3 py-2.5"
              >
                <svg
                  width="14"
                  height="14"
            ...
            <div
              class="sticky bottom-0 bg-denim-800 border-t border-denim-700/40 px-6 py-4 flex gap-3"
            >
    ...
    <Teleport to="body">
      <Transition name="modal">
        ...
        >
          <div class="absolute inset-0 bg-black/60 backdrop-blur-sm" />
          <div
            class="relative bg-denim-800 border border-denim-600/50 rounded-2xl shadow-2xl w-full max-w-sm"
          >
            <div
            ...
            <div v-if="detailEvent" class="px-5 py-4 space-y-2.5">
              <div class="flex justify-between items-center">
                ...
                <span class="text-xs font-mono text-caramel font-semibold">{{
                  detailEvent.equipmentId
                }}</span>
              </div>
              <div class="flex justify-between items-center">
                <span class="text-xs text-denim-200/50">Classification</span>
                <span
                  class="text-[10px] px-2 py-0.5 rounded border-l-2 font-medium"
                  :class="badgeClass(detailEvent.classification)"
                  >{{ detailEvent.classification }}</span
                >
              </div>
              <div class="flex justify-between items-center">
                <span class="text-xs text-denim-200/50">Interval</span
                ><span class="text-xs text-white">{{
                  detailEvent.interval
                }}</span>
              </div>
              <div class="flex justify-between items-center">
                <span class="text-xs text-denim-200/50">Date</span
                ><span class="text-xs text-white">{{ detailEvent.date }}</span>
              </div>
              <div class="flex justify-between items-center">
                <span class="text-xs text-denim-200/50">Technician</span
                ><span class="text-xs text-white">{{
                  detailEvent.technician
                }}</span>
              </div>
              <div
                v-if="detailEvent.woStatus"
                class="flex justify-between items-center"
              >
                <span class="text-xs text-denim-200/50">WO Status</span>
                <span
                  class="text-[10px] px-2 py-0.5 rounded font-semibold"
                  :class="woStatusBadge(detailEvent.woStatus)"
                  >{{ detailEvent.woStatus }}</span
                >
              </div>
              <div
                v-if="detailEvent.description"
                class="pt-2 border-t border-denim-700/30"
              >
            ...
            <div class="flex gap-2 px-5 pb-4 pt-2">
              ...
              <button
                class="btn-secondary flex-1 justify-center text-xs gap-1.5"
                @click="assignFromDetail(detailEvent)"
              >
                <svg
                  width="11"
                  height="11"
              ...
              <button
                ...
                @click="editEvent(detailEvent)"
              >
                Edit
              </button>
              <button
                class="btn-danger flex-1 justify-center text-xs"
                @click="deleteEvent(detailEvent)"
              >
                Delete
              </button>
            </div>
    ...
    <Teleport to="body">
      <Transition name="modal">
        <div
          ...
          @click.self="showMultiWO = false"
        >
          <div class="absolute inset-0 bg-black/60 backdrop-blur-sm" />
          <div
            class="relative bg-denim-800 border border-denim-600/50 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col"
          >
            <div
            ...
            >
              <div>
                <h2 class="text-base font-bold text-white">
                  Multiple WO Creation
                </h2>
                <p class="text-xs text-denim-200/50 mt-0.5">
                  Select schedules to create Work Orders in batch
                </p>
              </div>
              <button
                class="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-white/10 text-denim-200/50"
                @click="showMultiWO = false"
              >
                <svg
                  width="14"
                  height="14"
            ...
            <!-- Filter bar -->
            <div
              class="px-6 py-3 border-b border-denim-700/30 flex gap-2 flex-wrap"
            >
              <select
                v-model="multiFilter.classification"
                class="input text-xs py-1 max-w-[140px]"
              >
              ...
              <select
                v-model="multiFilter.interval"
                class="input text-xs py-1 max-w-[120px]"
              >
              ...
              <input
                v-model="multiFilter.search"
                class="input text-xs py-1 flex-1 min-w-[120px]"
            ...
            <div class="flex-1 overflow-y-auto divide-y divide-denim-700/15">
              <label
                v-for="ev in filteredMultiEvents"
                :key="ev.id"
              ...
              >
                <input
                  ...
                  :disabled="!!ev.woStatus"
                  class="accent-caramel w-4 h-4"
                />
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-2">
                    <span class="text-xs font-mono text-caramel">{{
                      ev.equipmentId
                    }}</span>
                    <span
                      class="text-[9px] px-1.5 py-0.5 rounded border-l-2 font-medium"
                      :class="badgeClass(ev.classification)"
                      >{{ ev.classification }}</span
                    >
                    <span
                      v-if="ev.woStatus"
                      class="text-[9px] px-1.5 py-0.5 rounded font-semibold"
                      :class="woStatusBadge(ev.woStatus)"
                      >{{ ev.woStatus }}</span
                    >
                  </div>
                  <p class="text-[10px] text-denim-200/40 mt-0.5">
                    {{ ev.date }} · {{ ev.interval }} · {{ ev.technician }}
                  </p>
                </div>
              </label>
              <div
                v-if="!filteredMultiEvents.length"
                class="px-6 py-10 text-center text-xs text-denim-200/25"
              >
                No schedules found
              </div>
            </div>
            ...
            <div
              class="px-6 py-4 border-t border-denim-700/40 flex items-center justify-between"
            >
              ...
              <div class="flex gap-2">
                ...
                >
                  ...
                  Create {{ multiSelected.length }} WO{{
...
const CLASS_MAP = new Map([
  [
    "Preventive",
    { border: "border-blue-500", text: "text-blue-300", bg: "bg-blue-500/10" },
  ],
  [
    "Corrective",
    {
...
function badgeClass(c: string) {
  ...
  return cls
    ...
    : "bg-denim-600/20 border-denim-500 text-denim-200";
}
function woStatusDot(s: string) {
  return WO_STATUS_MAP.get(s)?.dot ?? "bg-denim-400";
}
function woStatusBadge(s: string) {
  return WO_STATUS_MAP.get(s)?.badge ?? "bg-denim-600/40 text-denim-200";
}
...
const events = computed(() =>
  schedItems.value.map((s) => ({
    ...
    classification:
      actTypeToClass[(s as any).activityType ?? ""] ?? "Preventive",
...
function assignToTask() {
  ...
  const target = events.value.find(
    (e) =>
...
function openMultiWO() {
  multiSelected.value = [];
  showMultiWO.value = true;
}

const filteredMultiEvents = computed(() => {
  let list = events.value;
  if (multiFilter.value.classification)
    list = list.filter(
      (e) => e.classification === multiFilter.value.classification,
    );
  if (multiFilter.value.interval)
    list = list.filter((e) => e.interval === multiFilter.value.interval);
  if (multiFilter.value.search)
    list = list.filter((e) =>
      e.equipmentId
        .toLowerCase()
        .includes(multiFilter.value.search.toLowerCase()),
    );
  return list;
});
...
const duplicateWarning = computed(() => {
  if (!form.value.equipmentId || !form.value.classification || !form.value.date)
    return null;
  const dup = events.value.find(
    (e) =>
  ...
  if (editMode.value && editingId.value) {
    ...
    if (idx >= 0)
      events.value[idx] = {
        ...events.value[idx],
        ...form.value,
        technician: form.value.technicianId,
      };
  } else {
    events.value.push({
      id: Date.now(),
