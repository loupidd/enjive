  ...
  <div class="space-y-5">
    ...
    <div class="card p-4">
      ...
      <div class="mt-3 flex items-center gap-2">
        ...
        <span class="text-xs text-denim-200/50"
          >Showing data for
          <span class="text-caramel font-semibold">{{
            selectedMonthLabel
          }}</span></span
        >
      </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
      <div
        ...
        :key="kpi.label"
        class="card py-4 px-5 relative overflow-hidden"
      >
        <div
          class="absolute inset-0 flex items-end justify-end opacity-5 pointer-events-none"
        >
          <span class="text-8xl font-black leading-none">{{ kpi.value }}</span>
        </div>
        <p
          class="text-[10px] text-denim-200/40 uppercase tracking-wide font-semibold mb-1"
        >
          {{ kpi.label }}
        </p>
        ...
        <p
          ...
          :class="kpi.trendUp ? 'text-green-400' : 'text-red-400'"
        >
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      <div class="card p-0 overflow-hidden">
        ...
        >
          <div>
            <p class="text-sm font-bold text-white">
              Work Orders by Classification
            </p>
            <p class="text-[11px] text-denim-200/35 mt-0.5">
              {{ selectedMonthLabel }}
            </p>
          </div>
          <div class="flex gap-2 text-[10px] text-denim-200/40">
            <span
              v-for="l in CLASS_LEGEND"
              :key="l.label"
        ...
        <div class="p-5">
          ...
          <div v-if="chartType === 'bar'" class="space-y-3">
            <div
              v-for="row in classificationData"
              :key="row.label"
              class="group"
            >
              <div class="flex items-center justify-between mb-1">
                ...
                <span class="text-xs font-bold" :class="row.color">{{
                  row.value
                }}</span>
              </div>
              <div class="h-6 bg-denim-700/20 rounded-lg overflow-hidden">
                <div
                  class="h-full rounded-lg transition-all duration-700 flex items-center justify-end pr-2"
                  :class="row.barColor"
                  :style="{
                    width: Math.max(4, (row.value / maxClassVal) * 100) + '%',
                  }"
                >
          ...
          <div v-else-if="chartType === 'line'" class="space-y-4">
            <div v-for="row in classificationData" :key="row.label">
              <div class="flex items-center justify-between mb-1.5">
                <span
                  class="text-xs font-medium text-white flex items-center gap-2"
                >
                  <span
              ...
              >
                <polyline
                  ...
                  fill="none"
                  :stroke="row.lineStroke"
                  stroke-width="2"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                />
                <circle
                  v-for="(pt, i) in row.sparkline"
                  :key="i"
          ...
          <div
            ...
            class="flex flex-wrap gap-3 justify-center py-2"
          >
            <div
              v-for="row in classificationData"
              :key="row.label"
            ...
            >
              ...
              <span
                class="text-[9px] text-denim-200/50 text-center leading-tight max-w-[60px]"
                >{{ row.label }}</span
              >
            </div>
          </div>
        ...
        <div class="p-5 flex items-center gap-6">
          ...
          <div class="flex-1 space-y-2">
            ...
            >
              ...
              />
              <span class="text-xs text-denim-200/60 flex-1">{{
                seg.label
              }}</span>
      ...
      <div class="card p-0 overflow-hidden lg:col-span-2">
        ...
        >
          <div>
            ...
            <p class="text-[11px] text-denim-200/35 mt-0.5">
              Work Orders completed per month · last 6 months
            </p>
          </div>
          <div class="flex gap-3 text-[10px] text-denim-200/40">
            <span class="flex items-center gap-1"
              ><span
                class="w-3 h-1 rounded bg-caramel inline-block"
              />Preventive</span
            >
            <span class="flex items-center gap-1"
              ><span
                class="w-3 h-1 rounded bg-orange-400 inline-block"
              />Corrective</span
            >
            <span class="flex items-center gap-1"
              ><span
                class="w-3 h-1 rounded bg-green-400 inline-block"
              />Predictive</span
            >
            <span class="flex items-center gap-1"
              ><span
                class="w-3 h-1 rounded bg-purple-400 inline-block"
              />Thermography</span
            >
          </div>
        </div>
        <div class="p-5">
          <div class="flex items-end gap-2 h-48">
            ...
            >
              <!-- Stacked bar -->
              <div
                class="w-full flex flex-col-reverse gap-0.5 items-center"
                :style="{ height: '170px' }"
              >
                <div
                  class="w-full rounded-b transition-all duration-700"
                  :style="{
                    height: barPct(bar.preventive) + '%',
                    background: '#FFC677',
                  }"
                  :title="`Preventive: ${bar.preventive}`"
                />
                <div
                  class="w-full transition-all duration-700"
                  :style="{
                    height: barPct(bar.corrective) + '%',
                    background: '#fb923c',
                  }"
                  :title="`Corrective: ${bar.corrective}`"
                />
                <div
                  class="w-full transition-all duration-700"
                  :style="{
                    height: barPct(bar.predictive) + '%',
                    background: '#4ade80',
                  }"
                  :title="`Predictive: ${bar.predictive}`"
                />
                <div
                  class="w-full rounded-t transition-all duration-700"
                  :style="{
                    height: barPct(bar.thermography) + '%',
                    background: '#c084fc',
                  }"
                  :title="`Thermography: ${bar.thermography}`"
                />
              </div>
              <!-- Month label -->
              <span
                class="text-[9px] text-denim-200/50 mt-1 font-medium"
                :class="
                  bar.key === selectedMonth ? 'text-caramel font-bold' : ''
                "
                >{{ bar.month }}</span
              >
              <span
      ...
      <div class="card p-0 overflow-hidden">
        <div class="px-5 py-4 border-b border-denim-700/30">
          ...
          <p class="text-[11px] text-denim-200/35 mt-0.5">
            Based on latest work order results
          </p>
        </div>
        <div class="p-5 space-y-3">
          <div v-for="row in healthData" :key="row.label">
            <div class="flex items-center gap-3 mb-1.5">
              ...
              <span class="text-xs font-bold ml-auto" :class="row.textColor"
                >{{ row.count }} units</span
              >
      ...
      <div class="card p-0 overflow-hidden">
        <div class="px-5 py-4 border-b border-denim-700/30">
          ...
          <p class="text-[11px] text-denim-200/35 mt-0.5">
            Most reported this period · {{ selectedMonthLabel }}
          </p>
        </div>
        <div class="divide-y divide-denim-700/10">
          ...
          >
              ...
              :class="
                i === 0
                  ...
                  : i === 1
                    ...
                    : 'bg-denim-700/40 text-denim-200/40'
              "
            >
              {{ i + 1 }}
            </div>
            ...
            <!-- Mini bar -->
            <div
              class="w-16 h-1.5 bg-denim-700/20 rounded-full overflow-hidden"
            >
              <div
                class="h-full bg-red-500/60 rounded-full"
                :style="{
    ...
    <div class="card p-0 overflow-hidden">
      ...
      >
        <div>
          <p class="text-sm font-bold text-white">
            Completion Rate by Equipment Type
          </p>
          <p class="text-[11px] text-denim-200/35 mt-0.5">
            {{ selectedMonthLabel }}
          </p>
      ...
      <div class="overflow-x-auto">
        <table class="w-full min-w-[500px]">
          ...
          <tbody>
            <tr
              v-for="row in completionTable"
              :key="row.type"
              class="border-b border-denim-700/10 hover:bg-denim-700/10 transition-colors"
            >
              <td class="px-4 py-3 text-xs font-semibold text-white">
                {{ row.type }}
              </td>
              <td class="px-4 py-3 text-xs text-denim-200/60 font-mono">
                {{ row.total }}
              </td>
              <td class="px-4 py-3 text-xs text-green-400 font-semibold">
                {{ row.done }}
              </td>
              ...
              <td class="px-4 py-3">
                <div class="flex items-center gap-2">
                  <div
                    class="flex-1 h-2 bg-denim-700/20 rounded-full overflow-hidden"
                  >
                    <div
                      class="h-full rounded-full transition-all duration-700"
                      :class="
...
// ── Constants ─────────────────────────────────────────────────
// Equipment types populated from reports API data
const CHART_TYPES = [
...
const visibleMonths = computed(() =>
  MONTHS_DATA.slice(windowStart.value, windowStart.value + 5),
);
const selectedMonth = ref(MONTHS_DATA[MONTHS_DATA.length - 1].key);
const selectedMonthLabel = computed(
  () => MONTHS_DATA.find((m) => m.key === selectedMonth.value)?.label ?? "",
);
function shiftMonth(dir: number) {
  const newStart = Math.max(
    0,
    Math.min(MONTHS_DATA.length - 5, windowStart.value + dir),
  );
...
function apiMonthCount(key: string): number {
  return (
    apiReports.value?.workOrdersByMonth.find((m) => m.month === key)?.count ?? 0
  );
}
...
function monthData(key: string) {
  ...
  const byStatus = Object.fromEntries(
    r.workOrdersByStatus.map((s) => [s.status, s.count]),
  );
  const byType = Object.fromEntries(
    r.workOrdersByType.map((t) => [t.type, t.count]),
  );
  return {
...
const prev = computed(() =>
  monthData(MONTHS_DATA[Math.max(0, monthSeed(selectedMonth.value) - 1)].key),
);
const totalWOs = computed(
  () =>
...
const kpiCards = computed(() => {
  const c = current.value,
    p = prev.value;
  const pct = (n: number, o: number) =>
    o ? Math.round(((n - o) / o) * 100) + "%" : "—";
  return [
    {
      label: "Total WOs",
      value: totalWOs.value,
      color: "text-white",
      trendUp: totalWOs.value >= p.finish + p.process + p.waiting + p.rejected,
      trend: pct(totalWOs.value, p.finish + p.process + p.waiting + p.rejected),
    },
    {
      label: "Completed",
      value: c.finish,
      color: "text-green-400",
      trendUp: c.finish >= p.finish,
      trend: pct(c.finish, p.finish),
    },
    {
      label: "On Progress",
      value: c.process + c.waiting,
      color: "text-caramel",
      trendUp: false,
      trend: pct(c.process + c.waiting, p.process + p.waiting),
    },
    {
      label: "Rejected",
      value: c.rejected,
      color: "text-red-400",
      trendUp: c.rejected <= p.rejected,
      trend: pct(c.rejected, p.rejected),
    },
  ];
});
...
const classificationData = computed(() => {
  ...
  const sparkFor = (
    key: "preventive" | "corrective" | "predictive" | "thermography",
  ...
  return [
    ...
    },
    {
      ...
      lineStroke: "#4ade80",
      sparkline: sparkFor("predictive"),
    },
    {
      label: "Thermography Investigation",
      value: c.thermography,
      color: "text-purple-300",
      barColor: "bg-purple-500/70",
      lineStroke: "#c084fc",
      sparkline: sparkFor("thermography"),
    },
  ];
});
...
const maxClassVal = computed(() =>
  Math.max(...classificationData.value.map((r) => r.value), 1),
);
...
const trendMax = computed(() =>
  Math.max(
    ...trendBars.value.map(
      (b) => b.preventive + b.corrective + b.predictive + b.thermography,
    ),
    1,
  ),
);
function barPct(val: number) {
  return Math.round((val / trendMax.value) * 100);
}
...
function exportXls() {
  ...
  const header =
    "Equipment Type,Total WO,Finished,On Progress,Rejected,Completion Rate\n";
  const csv = rows
    .map(
      (r) =>
        `${r.type},${r.total},${r.done},${r.active},${r.rejected},${r.rate}%`,
    )
    .join("\n");
...
// Completion table from real API data
const completionTable = computed(() => {
  const r = apiReports.value;
  if (!r) return [];
  const byType = Object.fromEntries(
    r.workOrdersByType.map((t: any) => [t.type, t.count]),
  );
  return [
    {
      type: "Preventive",
      total: byType["PREVENTIVE"] ?? 0,
      done: byType["PREVENTIVE"] ?? 0,
      active: 0,
      rejected: 0,
      rate: 100,
    },
    {
      type: "Corrective",
      total: byType["CORRECTIVE"] ?? 0,
      done: byType["CORRECTIVE"] ?? 0,
      active: 0,
      rejected: 0,
      rate: 100,
    },
    {
      type: "Inspection",
      total: byType["INSPECTION"] ?? 0,
      done: byType["INSPECTION"] ?? 0,
      active: 0,
      rejected: 0,
      rate: 100,
    },
