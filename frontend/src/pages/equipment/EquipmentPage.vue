<template>
  <div class="space-y-4">
    ...
    <div class="relative">
      ...
      >
        <component :is="Search" :size="14" class="text-denim-200/40" />
      </div>
      <input
        v-model="searchQuery"
      ...
      >
        <button
          class="text-denim-200/40 hover:text-white transition-colors"
          @click="searchQuery = ''"
        >
          <svg
            width="12"
          ...
          >
            <td class="px-4 py-3">
              <div class="flex items-center gap-3">
                ...
                >
                  <component
                    :is="typeIcon(type.classification).icon"
                    :size="13"
                    class="shrink-0"
                  />
                </div>
...
function typeIcon(c: string) {
  const map: Record<string, { bg: string; icon: Component }> = {
    Mechanical: { bg: "bg-blue-500/15 text-blue-400", icon: Settings2 },
    Electrical: { bg: "bg-yellow-500/15 text-yellow-400", icon: Zap },
    Electronic: { bg: "bg-purple-500/15 text-purple-400", icon: Radio },
    Civil: { bg: "bg-green-500/15 text-green-400", icon: Building2 },
    Others: { bg: "bg-denim-600/50 text-denim-200", icon: Wrench },
  };
...
const filteredTypes = computed(() => {
  if (!searchQuery.value.trim()) return equipmentTypes.value;
  const q = searchQuery.value.toLowerCase();
  return equipmentTypes.value.filter((t) => t.name.toLowerCase().includes(q));
});
