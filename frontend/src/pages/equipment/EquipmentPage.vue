<template>
  <div class="space-y-4">
    ...
    <div class="flex items-center justify-between flex-wrap gap-3">
      <div>
        <h2 class="text-xl font-bold text-white">{{ t("nav.equipment") }}</h2>
            ...
            >
              <h3 class="font-bold text-white">
                {{
                  editingType
                    ? t("common.edit") + " Type"
                    : t("common.add") + " " + t("equipment.category")
                }}
              </h3>
              <button
...
function normalizeCategory(cat: string): string {
  ...
  if (
    ...
    c.includes("cubicle") ||
    c.includes("kontrol") ||
    c.includes("penerangan")
  )
    return "Panel Listrik";
  if (c.includes("genset") || c.includes("generator")) return "Genset";
  if (c.includes("pompa") || c.includes("pump") || c.includes("pmp"))
    return "Pompa";
  if (
    c.includes("cctv") ||
    c.includes("camera") ||
  ...
  if (
    ...
    c.includes("fingerprint")
  )
    return "Access Control";
  if (c.includes("lift") || c.includes("elevator") || c.includes("escalator"))
    return "Lift / Elevator";
  if (
    c.includes("fire") ||
    c.includes("tbfa") ||
    c.includes("hydrant") ||
    c.includes("sprinkler") ||
    c.includes("mcfa") ||
    c.includes("fa ")
  )
    return "Fire Alarm / APAR";
  if (c.includes("videophone") || c.includes("vdp") || c.includes("intercom"))
    return "Videophone";
  if (
    c.includes("tata suara") ||
    c.includes("tbts") ||
    c.includes("sound") ||
    c.includes("speaker") ||
    c.includes("audio")
  ...
  if (c.includes("ups") || c.includes("battery") || c.includes("batt"))
    return "UPS / Battery";
  if (
    c.includes("ihb") ||
    c.includes("tbtp") ||
    c.includes("power house") ||
    c.includes("mechanical")
  )
    return "Mechanical / Utilitas";
  if (
    c.includes("fresh air") ||
    c.includes("ventilasi") ||
    c.includes("exhaust") ||
    c.includes("fan")
  )
    return "Ventilasi / FAF";
  return cat || "Others";
}
  ...
  for (const eq of eqItems.value) {
    ...
    if (!groups[cat]) {
      groups[cat] = {
        id: cat,
        name: cat,
        classification: (eq as any).classification || "Others",
        total: 0,
...
function goToType(type: any) {
  router.push(`/equipment/type/${encodeURIComponent(type.name)}`);
}
