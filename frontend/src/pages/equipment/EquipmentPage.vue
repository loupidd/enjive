<template>
  <div class="space-y-4">
    ...
    <div class="flex items-center justify-between flex-wrap gap-3">
      <div><h2 class="text-xl font-bold text-white">{{ t("nav.equipment") }}</h2><div class="accent-bar mt-1.5"/></div>
      <div class="flex gap-2">
        <!-- Download Template -->
        <button class="btn-secondary text-xs px-3 py-1.5 gap-1.5" @click="downloadTemplate">
        ...
        <!-- Import XLS -->
        <button class="btn-secondary text-xs px-3 py-1.5 gap-1.5" @click="showXlsImport=true">
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="12" y1="18" x2="12" y2="12"/><line x1="9" y1="15" x2="12" y2="18"/><line x1="15" y1="15" x2="12" y2="18"/></svg>
          Import XLS
        </button>
    ...
    <div class="max-w-sm">
      <GlobalSearch placeholder="Search equipment ID or name..." :sources="['equipment']"/>
    </div>

    <!-- Loading / error -->
    <div v-if="eqLoading" class="flex items-center gap-2 text-xs text-denim-200/40 py-2">
    ...
    <div v-if="eqError" class="text-xs text-red-400 bg-red-500/10 rounded-lg px-3 py-2">{{ eqError }}</div>

    <!-- {{ t("equipment.category") }}s Table -->
    <div class="card p-0 overflow-hidden">
      <table class="w-full text-sm">
        <thead>
          <tr class="border-b border-denim-700/40">
            <th class="text-left px-4 py-3 text-xs font-semibold text-denim-200/60 uppercase tracking-wide">{{ t("equipment.category") }}</th>
            <th class="text-center px-4 py-3 text-xs font-semibold text-denim-200/60 uppercase tracking-wide">Total Equipment</th>
            <th class="text-left px-4 py-3 text-xs font-semibold text-denim-200/60 uppercase tracking-wide">{{ t("equipment.maintainer") }}</th>
            <th class="px-4 py-3 text-xs font-semibold text-denim-200/60 uppercase tracking-wide text-right">Action</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="type in equipmentTypes" :key="type.id" class="border-b border-denim-700/20 hover:bg-denim-700/20 transition-colors cursor-pointer" @click="goToType(type)">
            <td class="px-4 py-3">
              <div class="flex items-center gap-3">
                <div class="w-8 h-8 rounded-lg flex items-center justify-center" :class="typeIcon(type.classification).bg">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" :stroke="typeIcon(type.classification).stroke" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <td class="px-4 py-3 text-center"><span class="text-lg font-bold text-caramel">{{ type.total }}</span></td>
        ...
        <div v-if="showAddType" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showAddType=false">
          ...
          <div class="relative bg-denim-800 border border-denim-600/40 rounded-2xl shadow-2xl w-full max-w-sm">
            <div class="flex items-center justify-between px-5 py-4 border-b border-denim-700/40">
              <h3 class="font-bold text-white">{{ editingType ? "Edit Type" : "New {{ t("equipment.category") }}" }}</h3>
            ...
            <div class="px-5 pb-4 flex justify-end gap-2">
              <button class="btn-secondary" @click="showAddType=false">{{ t("common.cancel") }}</button>
          ...
          <div class="relative bg-denim-800 border border-denim-600/40 rounded-2xl shadow-2xl w-full max-w-2xl">
            ...
            <div class="px-6 py-5 space-y-4">
              ...
              <div class="grid grid-cols-2 gap-3">
                ...
                <div><label class="label">{{ t("equipment.category") }} <span class="text-red-400">*</span></label>
              ...
              <div class="grid grid-cols-2 gap-3">
                ...
                <div><label class="label">{{ t("equipment.maintainer") }} <span class="text-red-400">*</span></label>
            ...
            <div class="sticky bottom-0 bg-denim-800 border-t border-denim-700/40 px-6 py-4 flex justify-end gap-2 rounded-b-2xl">
              <button class="btn-secondary" @click="showAddEquipment=false">{{ t("common.cancel") }}</button>
    ...
    <Teleport to="body">
      <Transition name="modal">
        <div v-if="showXlsImport" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="closeXlsImport">
          <div class="absolute inset-0 bg-black/60 backdrop-blur-sm"/>
          <div class="relative bg-denim-800 border border-denim-600/40 rounded-2xl shadow-2xl w-full max-w-2xl">
            ...
            <div class="flex items-center justify-between px-6 py-4 border-b border-denim-700/40">
              <div>
                ...
                <p class="text-xs text-denim-200/40 mt-0.5">Upload a spreadsheet to bulk-import equipment records</p>
              </div>
              <button class="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-white/10 text-denim-200/50" @click="closeXlsImport">✕</button>
            </div>
            ...
            <div class="px-6 py-5 space-y-5">
              ...
              <!-- Format info -->
              <div class="bg-denim-700/20 rounded-xl p-4 border border-denim-700/30">
                <p class="text-xs font-semibold text-white mb-2">Accepted column headers</p>
                <div class="grid grid-cols-2 gap-x-8 gap-y-1 text-[11px] text-denim-200/60">
                  <div v-for="col in TEMPLATE_COLS" :key="col.key" class="flex items-center gap-1.5">
                    <span :class="col.required ? 'text-red-400' : 'text-denim-200/30'">●</span>
                    <span class="font-mono text-caramel/70">{{ col.key }}</span>
                    <span class="text-denim-200/40">{{ col.aliases.length ? '/ '+col.aliases.join(' / ') : '' }}</span>
                    <span v-if="col.required" class="text-red-400 ml-auto">required</span>
                  </div>
                </div>
                <p class="text-[10px] text-denim-200/30 mt-3">
                  <span class="text-red-400">●</span> Required &nbsp;
                  <span class="text-denim-200/30">●</span> Optional &nbsp;·&nbsp;
                  Column headers are case-insensitive and flexible (aliases accepted).
                </p>
              </div>
              ...
              <div
                ...
                :class="xlsDragOver ? 'border-caramel/60 bg-caramel/5' : 'border-denim-600/40 hover:border-denim-500/60'"
              ...
              >
                <input ref="xlsFileInput" type="file" accept=".xlsx,.xls,.csv" class="hidden" @change="onXlsFile"/>
                <svg class="mx-auto mb-3" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" :class="xlsDragOver ? 'text-caramel' : 'text-denim-200/30'">
                  <path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/>
                  <line x1="12" y1="18" x2="12" y2="12"/><line x1="9" y1="15" x2="12" y2="12"/><line x1="15" y1="15" x2="12" y2="12"/>
                </svg>
                <p class="text-sm text-denim-200/60">Drop your <span class="text-caramel">.xlsx / .xls / .csv</span> file here</p>
                <p class="text-xs text-denim-200/30 mt-1">or click to browse</p>
              </div>

              <!-- Parse errors -->
              <div v-if="xlsErrors.length" class="bg-red-500/8 border border-red-500/20 rounded-xl px-4 py-3 space-y-1">
                <p class="text-xs font-semibold text-red-400 mb-1">Issues found ({{ xlsErrors.length }})</p>
                <p v-for="(err,i) in xlsErrors.slice(0,8)" :key="i" class="text-[11px] text-red-300/80">{{ err }}</p>
                <p v-if="xlsErrors.length > 8" class="text-[11px] text-red-300/50">…and {{ xlsErrors.length-8 }} more</p>
              </div>

              <!-- Preview table -->
              <div v-if="xlsPreview.length" class="overflow-x-auto">
                <p class="text-xs text-denim-200/50 mb-2">Preview — {{ xlsPreview.length }} row{{ xlsPreview.length!==1?'s':'' }} detected
                  <span class="text-caramel ml-1">({{ xlsValid.length }} valid, {{ xlsPreview.length - xlsValid.length }} skipped)</span>
                </p>
                <table class="w-full text-[11px] border-collapse">
                  <thead>
                    <tr class="text-denim-200/50 text-left border-b border-denim-700/30">
                  ...
                  <tbody>
                    <tr v-for="(row,i) in xlsPreview.slice(0,10)" :key="i"
                      class="border-b border-denim-700/15"
                      :class="row._valid ? '' : 'opacity-40'"
                    >
                      <td class="py-1.5 pr-3 font-mono text-caramel/80">{{ row.id || '—' }}</td>
                      <td class="py-1.5 pr-3 text-white">{{ row.name || '—' }}</td>
                      <td class="py-1.5 pr-3 text-denim-200/60">{{ row.classification || '—' }}</td>
                      <td class="py-1.5 pr-3 text-denim-200/60">{{ row.type || '—' }}</td>
                      <td class="py-1.5 pr-3 text-denim-200/60">{{ row.section || '—' }}</td>
                      <td class="py-1.5">
                        <span v-if="row._valid" class="text-green-400 text-[10px]">✓ OK</span>
                        <span v-else class="text-red-400 text-[10px]" :title="row._error">✗ {{ row._error }}</span>
                      </td>
                    </tr>
                  </tbody>
                </table>
                <p v-if="xlsPreview.length > 10" class="text-[10px] text-denim-200/30 mt-1">Showing first 10 of {{ xlsPreview.length }} rows</p>
              </div>
            </div>

            <!-- Footer -->
            <div class="px-6 pb-5 flex items-center justify-between gap-3 border-t border-denim-700/30 pt-4">
              ...
              <div class="flex gap-2">
                <button class="btn-secondary" @click="closeXlsImport">{{ t("common.cancel") }}</button>
                <button class="btn-primary" :disabled="!xlsValid.length" @click="confirmXlsImport">
...
const { items: eqItems, loading: eqLoading, error: eqError, fetchAll } = useEquipment()

// fetchAll paginates all equipment so we get the full 4,660
onMounted(() => fetchAll())
...
// Consolidate sub-categories into main groups for display
function normalizeCategory(cat: string): string {
  const c = cat.toLowerCase()
  if (c.includes("ac") || c.includes("hvac") || c.includes("chiller") || c.includes("ahu") || c.includes("fcu")) return "AC / HVAC"
  if (c.includes("panel") || c.includes("lvmd") || c.includes("mdp") || c.includes("sdp") || c.includes("pp ") || c.includes("cubicle") || c.includes("kontrol") || c.includes("penerangan")) return "Panel Listrik"
  if (c.includes("genset") || c.includes("generator")) return "Genset"
  if (c.includes("pompa") || c.includes("pump") || c.includes("pmp")) return "Pompa"
  if (c.includes("cctv") || c.includes("camera") || c.includes("dvr") || c.includes("nvr")) return "CCTV"
  if (c.includes("access") || c.includes("acces") || c.includes("boomgate") || c.includes("turnstile") || c.includes("fingerprint")) return "Access Control"
  if (c.includes("lift") || c.includes("elevator") || c.includes("escalator")) return "Lift / Elevator"
  if (c.includes("fire") || c.includes("tbfa") || c.includes("hydrant") || c.includes("sprinkler") || c.includes("mcfa") || c.includes("fa ")) return "Fire Alarm / APAR"
  if (c.includes("videophone") || c.includes("vdp") || c.includes("intercom")) return "Videophone"
  if (c.includes("tata suara") || c.includes("tbts") || c.includes("sound") || c.includes("speaker") || c.includes("audio")) return "Tata Suara"
  if (c.includes("thermo")) return "Thermographic"
  if (c.includes("transformer") || c.includes("trafo")) return "Transformer"
  if (c.includes("ups") || c.includes("battery") || c.includes("batt")) return "UPS / Battery"
  if (c.includes("ihb") || c.includes("tbtp") || c.includes("power house") || c.includes("mechanical")) return "Mechanical / Utilitas"
  if (c.includes("fresh air") || c.includes("ventilasi") || c.includes("exhaust") || c.includes("fan")) return "Ventilasi / FAF"
  return cat || "Others"
}

// Group by raw category name from DB — matches old system type names exactly
const equipmentTypes = computed(() => {
  const groups: Record<string, any> = {}
  for (const eq of eqItems.value) {
    const cat = eq.category || "Others"
    if (!groups[cat]) {
      groups[cat] = { id: cat, name: cat, classification: (eq as any).classification || "Others", total: 0, maintainer: "PT Sumber Sarana Solusindo" }
    }
    groups[cat].total++
  }
...
function goToType(type: any) { router.push(`/equipment/type/${encodeURIComponent(type.name)}`) }
...
function getLocation() { navigator.geolocation?.getCurrentPosition(p=>{eqForm.value.lat=String(p.coords.latitude);eqForm.value.lng=String(p.coords.longitude)}) }
function submitEquipment() { showAddEquipment.value=false }

// ── XLS Import ──────────────────────────────────────────────────

// Column definitions: key = canonical name, aliases = other accepted names
const TEMPLATE_COLS = [
  { key:'id',             aliases:['equipment_id','eq_id','asset_id','asset id','equipment id'],             required:true  },
  { key:'name',           aliases:['device_name','device name','equipment_name','nama','nama alat'],          required:true  },
  { key:'classification', aliases:['class','klasifikasi','category'],                                         required:true  },
  { key:'type',           aliases:['type_name','equipment_type','tipe','jenis'],                               required:true  },
  { key:'section',        aliases:['area','zone','lokasi','sub_area'],                                         required:true  },
  { key:'critical_level', aliases:['critical','level','criticality','critical level'],                        required:false },
  { key:'location',       aliases:['lokasi_detail','floor','lantai'],                                         required:false },
  { key:'model',          aliases:['model_number','model no','no model'],                                     required:false },
  { key:'serial',         aliases:['serial_number','serial no','no serial','sn'],                             required:false },
  { key:'brand',          aliases:['merk','maker','brand/maker'],                                             required:false },
  { key:'vendor',         aliases:['supplier','pemasok'],                                                     required:false },
  { key:'install_date',   aliases:['installed','tanggal_pasang','install date','tanggal pasang'],              required:false },
  { key:'capacity',       aliases:['size','ukuran','kapasitas'],                                              required:false },
  { key:'maintainer',     aliases:['maintenance_by','maintained by','perawatan'],                             required:false },
]
...
function normaliseHeader(h: string): string {
  ...
  for (const col of TEMPLATE_COLS) {
    ...
    if (col.aliases.map(a => a.toLowerCase().replace(/[\s_-]+/g,'_')).includes(lower)) return col.key
  }
  return lower
}
...
/).map(l => l.split(',').map(c => c.replace(/^"|"$/g, '').trim()))
  ...
  try {
    ...
    xlsErrors.value = ['Failed to parse file. Please use .xlsx, .xls or .csv format.']
  }
}
...
function confirmXlsImport() {
  ...
  imported.forEach(eq => {
    const exists = equipmentTypes.value.some(t => t.name.toLowerCase() === eq.type.toLowerCase())
    if (!exists && eq.type) {
...
function downloadTemplate() {
  ...
  const example = 'EDA_AC_001,AC Function Room,Mechanical,AC Fasilitas,Air Conditioner,L2,Lantai 2,Samsung VRF,SN-123,Samsung,PT ABC,2020-01-15,5PK,PT Sumber Sarana Solusindo'
  const csv = `${headers}
${example}
`
  const url = URL.createObjectURL(new Blob([csv], { type:'text/csv' }))
  const a   = document.createElement('a')
  a.href = url; a.download = 'enjive_equipment_template.csv'; a.click()
