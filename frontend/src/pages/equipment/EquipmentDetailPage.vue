  ...
  <div class="space-y-4">
    ...
    <div class="flex items-start gap-3 flex-wrap justify-between">
        ...
        <div>
          <div class="flex items-center gap-2 flex-wrap">
            ...
            <span
              ...
              :class="criticalClass(eq.criticalLevel)"
            >
              Critical L{{ eq.criticalLevel }}
            </span>
            <span
              class="text-[10px] px-2 py-0.5 rounded-full bg-green-500/15 text-green-400 border border-green-500/20 font-semibold"
            >
              {{ eq.status }}
            </span>
          </div>
      ...
      <div class="flex items-center gap-1.5 flex-wrap">
        ...
        <button
          class="relative btn-secondary px-3 py-1.5 text-xs gap-1.5 border-red-500/30 text-red-400"
          @click="activeTab = 'active-wo'"
        >
          <span
            class="absolute -top-0.5 -right-0.5 w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse"
          />
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
      <div class="card py-3 px-4 flex items-center gap-3">
        <div
          class="w-8 h-8 rounded-lg bg-caramel/10 flex items-center justify-center shrink-0"
        >
    ...
    <div class="grid grid-cols-1 lg:grid-cols-5 gap-4">
      ...
      <div class="lg:col-span-2 space-y-4">
        ...
        <div class="card p-0 overflow-hidden">
          <div
            class="aspect-[4/3] bg-gradient-to-br from-denim-900/80 to-denim-800/60 flex flex-col items-center justify-center gap-2"
          >
            <div
              class="w-12 h-12 rounded-xl bg-denim-700/50 flex items-center justify-center"
            >
        ...
        <div class="card p-0 overflow-hidden">
          <div class="aspect-video bg-denim-900/60 relative overflow-hidden">
            ...
            >
              <div
                class="w-7 h-7 rounded-full bg-red-500 shadow-lg shadow-red-500/40 flex items-center justify-center"
              >
                <IconMapPin :size="14" class="text-white" />
              </div>
              <div
                class="bg-denim-800/80 backdrop-blur-sm rounded-lg px-3 py-1.5 text-center"
              >
                <p class="text-[10px] font-mono text-caramel/80">
                  {{ eq.lat }}, {{ eq.lng }}
                </p>
          ...
          <div
            class="px-3 py-2 flex justify-between items-center border-t border-denim-700/30"
          >
      ...
      <div class="lg:col-span-3 space-y-3">
        ...
        <div class="card p-0 overflow-hidden">
          ...
          >
            ...
            <div class="flex items-center gap-3">
              <span class="text-[10px] text-denim-200/25">{{
                showSpecs ? "Collapse" : "Expand"
              }}</span>
              <IconChevronDown
                :size="14"
                class="text-denim-200/40 transition-transform duration-200"
                :class="showSpecs ? 'rotate-180' : ''"
              />
            </div>
          </button>
          <Transition name="collapse">
            <div
              v-if="showSpecs"
              class="border-t border-denim-700/20 px-4 py-3"
            >
              <div class="grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-3">
                <div v-for="row in specRows" :key="row.label">
                  <p class="text-[10px] text-denim-200/35 leading-none mb-0.5">
                    {{ row.label }}
                  </p>
        ...
        <div class="card p-0 overflow-hidden">
          <!-- Tab nav -->
          <div
            class="flex overflow-x-auto border-b border-denim-700/40 bg-denim-900/20"
          >
            <button
              v-for="tab in TABS"
              :key="tab.key"
            ...
            <table class="w-full text-xs min-w-[560px]">
              <thead>
                <tr class="border-b border-denim-700/30 bg-denim-900/20">
                  <th
                    ...
                    :key="h"
                    class="px-3 py-2 text-left text-[10px] font-semibold text-denim-200/40 uppercase tracking-wide"
                  >
                    {{ h }}
                  </th>
                </tr>
              ...
              <tbody>
                ...
                >
                  ...
                  <td
                    class="px-3 py-2.5 font-mono text-caramel text-[10px] hover:underline"
                  >
                    {{ r.id }}
                  </td>
                  ...
                  <td class="px-3 py-2.5 text-denim-100/60">
                    {{ r.interval }}
                  </td>
                  <td class="px-3 py-2.5">
                    <span
                ...
                <tr v-if="!maintenanceRecords.length">
                  <td
                    colspan="8"
                    class="px-3 py-8 text-center text-denim-200/25"
                  >
            ...
            <table class="w-full text-xs min-w-[640px]">
              ...
              <tbody>
                ...
                >
                  ...
                  <td
                    class="px-3 py-2.5 text-white font-medium max-w-[120px] truncate"
                  >
                    {{ r.name }}
                  </td>
                  <td class="px-3 py-2.5 text-denim-100/60">{{ r.date }}</td>
                  <td class="px-3 py-2.5">
                    <span
                      class="px-1.5 py-0.5 rounded text-[10px]"
                      :class="opColor(r.operation)"
                      >{{ r.operation }}</span
                    >
                  </td>
                  <td
                    class="px-3 py-2.5 text-denim-100/60 max-w-[100px] truncate"
                    :title="r.condition"
                  >
                    {{ r.condition }}
                  </td>
          ...
          <div v-else-if="activeTab === 'activities'" class="p-4 space-y-2">
            ...
            >
              <span
                class="text-[10px] text-denim-200/25 w-4 text-right shrink-0"
                >{{ i + 1 }}</span
              >
              ...
              <span
                ...
                :class="
                  a.status === 'Enable'
                    ? 'bg-green-500/15 text-green-400'
                    : 'bg-denim-600/40 text-denim-200/40'
                "
                >{{ a.status }}</span
              >
            </div>
            <p
              ...
              class="text-xs text-denim-200/30 text-center py-8"
            >
              {{ t("common.noData") }}
            </p>
          </div>
          ...
          <div v-else-if="activeTab === 'stats'" class="p-4 space-y-4">
            ...
            <div>
              <p
                class="text-[10px] font-semibold text-denim-200/40 uppercase tracking-wide mb-2"
              >
                Failure History
              </p>
              <table class="w-full text-xs">
                <thead>
                  <tr class="border-b border-denim-700/30">
                    <th
                      ...
                      :key="h"
                      class="py-1.5 text-left text-[10px] text-denim-200/30 uppercase tracking-wide px-1"
                    >
                      {{ h }}
                    </th>
                  </tr>
                ...
                <tbody>
                  <tr>
                    <td colspan="6" class="py-6 text-center text-denim-200/25">
                      No failure history
                    </td>
                  </tr>
                </tbody>
              ...
              <div
                class="mt-3 text-[10px] text-denim-200/25 space-y-0.5 border-t border-denim-700/20 pt-3"
              >
                <p>
                  • MTBF = Mean Time Before Failure &nbsp;·&nbsp; MTTR = Mean
                  Time to Repair
                </p>
                <p>
                  • PM rutin diabaikan · PM besar (&gt;2x24 jam) diperhitungkan
                  sebagai downtime
                </p>
              </div>
            </div>
      ...
      <Transition name="modal">
        ...
        >
          ...
          >
            ...
            >
              <div class="flex items-center gap-2">
                <IconQrCode :size="14" class="text-caramel/60" />
                <h3 class="font-bold text-white text-sm">QR Code</h3>
              </div>
              <button
            ...
            <div class="p-6">
              <div
                class="w-44 h-44 mx-auto bg-white rounded-xl p-2 mb-4 flex items-center justify-center"
              >
                <canvas ref="qrCanvas" width="160" height="160" />
              </div>
              <p class="text-[10px] text-denim-200/40 mb-1 text-center">
                Scans to:
              </p>
              <p
                class="text-xs font-mono text-caramel font-bold text-center break-all"
              >
                {{ qrUrl }}
              </p>
              <p class="text-xs text-denim-200/50 mt-1 text-center">
                {{ eq.name }}
              </p>
              <button
                class="btn-secondary w-full justify-center mt-4 text-xs gap-1.5"
                @click="downloadQR"
              >
                <IconDownload :size="12" />{{ t("common.export") }} QR
              </button>
            </div>
          </div>
        </div>
