  ...
  <div class="space-y-4">
    ...
    <div class="flex items-start justify-between flex-wrap gap-3">
      <div class="flex items-center gap-3">
        <button class="flex items-center gap-1.5 text-sm text-caramel hover:text-caramel/80" @click="router.back()">
      ...
      <div class="flex flex-wrap gap-2">
        <button @click="activeTab='maintenance'" class="btn-secondary text-xs px-3 py-1.5" :class="activeTab==='maintenance'?'ring-1 ring-caramel/40':''">Maintenance Record</button>
        <button @click="activeTab='trouble'" class="btn-secondary text-xs px-3 py-1.5" :class="activeTab==='trouble'?'ring-1 ring-caramel/40':''">Trouble Record</button>
        <button @click="activeTab='activities'" class="btn-secondary text-xs px-3 py-1.5" :class="activeTab==='activities'?'ring-1 ring-caramel/40':''">Activities</button>
        <button @click="activeTab='active-wo'" class="relative btn-secondary text-xs px-3 py-1.5 border-red-500/40 text-red-400" :class="activeTab==='active-wo'?'ring-1 ring-red-400/40':''">
          <span class="absolute -top-1 -right-1 w-2 h-2 rounded-full bg-red-500 animate-pulse"/>
          Active WO
        </button>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-5 gap-4">
      ...
      <div class="lg:col-span-2 space-y-4">
        ...
        <div class="card p-0 overflow-hidden">
          <div class="aspect-video bg-denim-900/60 flex flex-col items-center justify-center text-denim-200/30">
        ...
        <!-- Map placeholder -->
        <div class="card p-0 overflow-hidden">
          <div class="aspect-video bg-denim-900/60 relative flex items-center justify-center">
            <!-- Fake map grid -->
            <div class="absolute inset-0 opacity-10">
              <svg width="100%" height="100%"><defs><pattern id="grid" width="20" height="20" patternUnits="userSpaceOnUse"><path d="M 20 0 L 0 0 0 20" fill="none" stroke="#FFC677" stroke-width="0.5"/></pattern></defs><rect width="100%" height="100%" fill="url(#grid)"/></svg>
            </div>
            <div class="text-center z-10">
              <div class="w-6 h-6 rounded-full bg-red-500 shadow-lg shadow-red-500/50 mx-auto mb-2 flex items-center justify-center text-white text-xs">📍</div>
              <p class="text-xs text-denim-200/60">{{ eq.lat }}, {{ eq.lng }}</p>
      ...
      <div class="lg:col-span-3 space-y-4">
        ...
        <div class="card p-0 overflow-hidden">
          <table class="w-full text-sm">
            <tbody>
              <tr v-for="row in identityRows" :key="row.label" class="border-b border-denim-700/20 last:border-0">
                <td class="px-4 py-2.5 text-denim-200/50 font-medium text-xs w-40 shrink-0">{{ row.label }}</td>
                <td class="px-4 py-2.5 text-white font-medium">{{ row.value || "—" }}</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div class="card p-0 overflow-hidden">
          ...
          <div v-if="activeTab==='maintenance'" class="overflow-x-auto">
            <table class="w-full text-xs min-w-[600px]">
              <thead><tr class="border-b border-denim-700/30">
                <th class="px-3 py-2 text-left text-denim-200/50 uppercase tracking-wide">No</th>
                <th class="px-3 py-2 text-left text-denim-200/50 uppercase tracking-wide">ID Maintenance</th>
                <th class="px-3 py-2 text-left text-denim-200/50 uppercase tracking-wide">Date</th>
                <th class="px-3 py-2 text-left text-denim-200/50 uppercase tracking-wide">Classification</th>
                <th class="px-3 py-2 text-left text-denim-200/50 uppercase tracking-wide">Interval</th>
                <th class="px-3 py-2 text-left text-denim-200/50 uppercase tracking-wide">WO Status</th>
                <th class="px-3 py-2 text-left text-denim-200/50 uppercase tracking-wide">Operation</th>
                <th class="px-3 py-2 text-left text-denim-200/50 uppercase tracking-wide">Healthy</th>
              </tr></thead>
              <tbody>
                <tr v-for="(r,i) in maintenanceRecords" :key="r.id" class="border-b border-denim-700/10 hover:bg-denim-700/10">
                  ...
                  <td class="px-3 py-2"><span class="px-1.5 py-0.5 rounded text-[10px] font-medium" :class="classColor(r.classification)">{{ r.classification }}</span></td>
                  <td class="px-3 py-2 text-denim-100/70">{{ r.interval }}</td>
                  <td class="px-3 py-2"><span class="px-1.5 py-0.5 rounded text-[10px]" :class="woColor(r.woStatus)">{{ r.woStatus }}</span></td>
                  <td class="px-3 py-2 text-denim-100/70">{{ r.operation }}</td>
                  <td class="px-3 py-2"><span class="px-1.5 py-0.5 rounded text-[10px]" :class="r.healthy==='Good'?'bg-green-500/15 text-green-400':'bg-red-500/15 text-red-400'">{{ r.healthy }}</span></td>
                </tr>
                <tr v-if="!maintenanceRecords.length"><td colspan="8" class="px-3 py-6 text-center text-denim-200/30">No maintenance records</td></tr>
              </tbody>
            </table>
          </div>
          ...
          <div v-else-if="activeTab==='trouble'" class="overflow-x-auto">
            <table class="w-full text-xs min-w-[700px]">
              <thead><tr class="border-b border-denim-700/30">
                <th class="px-3 py-2 text-left text-denim-200/50 uppercase tracking-wide">No</th>
                <th class="px-3 py-2 text-left text-denim-200/50 uppercase tracking-wide">ID Kerusakan</th>
                <th class="px-3 py-2 text-left text-denim-200/50 uppercase tracking-wide">Nama Kerusakan</th>
                <th class="px-3 py-2 text-left text-denim-200/50 uppercase tracking-wide">Tanggal</th>
                <th class="px-3 py-2 text-left text-denim-200/50 uppercase tracking-wide">Operasi</th>
                <th class="px-3 py-2 text-left text-denim-200/50 uppercase tracking-wide">Kondisi</th>
                <th class="px-3 py-2 text-left text-denim-200/50 uppercase tracking-wide">Dampak</th>
                <th class="px-3 py-2 text-left text-denim-200/50 uppercase tracking-wide">Reporter</th>
                <th class="px-3 py-2 text-left text-denim-200/50 uppercase tracking-wide">Status</th>
              </tr></thead>
              <tbody>
                <tr v-for="(r,i) in troubleRecords" :key="r.id" class="border-b border-denim-700/10 hover:bg-denim-700/10">
                  ...
                  <td class="px-3 py-2 text-denim-100/70">{{ r.date }}</td>
                  <td class="px-3 py-2 text-denim-100/70">{{ r.operation }}</td>
                  <td class="px-3 py-2 text-denim-100/70 max-w-[120px] truncate" :title="r.condition">{{ r.condition }}</td>
                  <td class="px-3 py-2 text-denim-100/70 max-w-[120px] truncate" :title="r.impact">{{ r.impact }}</td>
                  <td class="px-3 py-2 text-denim-100/70">{{ r.reporter }}</td>
                  <td class="px-3 py-2"><span class="px-1.5 py-0.5 rounded text-[10px] font-medium" :class="tStatus(r.status)">{{ r.status }}</span></td>
                </tr>
                <tr v-if="!troubleRecords.length"><td colspan="9" class="px-3 py-6 text-center text-denim-200/30">No matching records found</td></tr>
              </tbody>
            </table>
          </div>
          ...
          <div v-else-if="activeTab==='activities'" class="p-4 space-y-2">
            <div v-for="(a,i) in eqActivities" :key="a.id" class="flex items-center gap-3 p-3 rounded-lg bg-denim-900/40 border border-denim-700/20">
              <span class="text-xs text-denim-200/30 w-4 text-right shrink-0">{{ i+1 }}</span>
              <div class="flex-1 min-w-0">
                <p class="text-sm font-medium text-white">{{ a.name }}</p>
                <p class="text-xs text-denim-200/50">{{ a.type }} · {{ a.answerType }} <span v-if="a.unit">· {{ a.unit }}</span></p>
              </div>
              <span class="text-[10px] px-2 py-0.5 rounded font-medium" :class="a.status==='Enable'?'bg-green-500/15 text-green-400':'bg-denim-600/50 text-denim-200/40'">{{ a.status }}</span>
            </div>
            <p v-if="!eqActivities.length" class="text-xs text-denim-200/30 text-center py-6">No activities</p>
          </div>
          ...
          <div v-else-if="activeTab==='active-wo'" class="overflow-x-auto">
            <table class="w-full text-xs min-w-[600px]">
              <thead><tr class="border-b border-denim-700/30">
                ...
                <th class="px-3 py-2 text-left text-denim-200/50 uppercase tracking-wide">Interval</th>
                <th class="px-3 py-2 text-left text-denim-200/50 uppercase tracking-wide">WO Status</th>
                <th class="px-3 py-2 text-left text-denim-200/50 uppercase tracking-wide">Technician</th>
              </tr></thead>
              <tbody>
                <tr v-for="(r,i) in activeWOs" :key="r.id" class="border-b border-denim-700/10 hover:bg-denim-700/10">
                  ...
                  <td class="px-3 py-2 font-mono text-caramel">{{ r.id }}</td>
                  <td class="px-3 py-2 text-denim-100/70">{{ r.date }}</td>
                  <td class="px-3 py-2"><span class="px-1.5 py-0.5 rounded text-[10px]" :class="classColor(r.classification)">{{ r.classification }}</span></td>
                  <td class="px-3 py-2 text-denim-100/70">{{ r.interval }}</td>
                  <td class="px-3 py-2"><span class="px-1.5 py-0.5 rounded text-[10px] animate-pulse" :class="woColor(r.woStatus)">{{ r.woStatus }}</span></td>
                  <td class="px-3 py-2 text-denim-100/70">{{ r.technician }}</td>
                </tr>
                <tr v-if="!activeWOs.length"><td colspan="7" class="px-3 py-6 text-center text-denim-200/30">No active work orders</td></tr>
              </tbody>
            </table>
          </div>
          ...
          <div v-else-if="activeTab==='stats'" class="p-4 space-y-4">
            <table class="w-full text-sm">
              <tbody>
                <tr v-for="row in statsRows" :key="row.label" class="border-b border-denim-700/20 last:border-0">
                  <td class="py-2 text-denim-200/50 text-xs">{{ row.label }}</td>
                  <td class="py-2 text-denim-200/50 text-center px-2">:</td>
                  <td class="py-2 text-white font-medium text-xs">{{ row.value }}</td>
                </tr>
              </tbody>
            </table>
            <!-- Failure History -->
            <div>
              <p class="text-xs font-semibold text-denim-200/60 uppercase tracking-wide mb-2">Failure History</p>
              <table class="w-full text-xs">
                <thead><tr class="border-b border-denim-700/30">
                  <th class="py-1.5 text-left text-denim-200/40">No</th>
                  <th class="py-1.5 text-left text-denim-200/40">Tanggal Down</th>
                  <th class="py-1.5 text-left text-denim-200/40">Tanggal Up</th>
                  <th class="py-1.5 text-left text-denim-200/40">Total Downtime</th>
                  <th class="py-1.5 text-left text-denim-200/40">Kerusakan</th>
                  <th class="py-1.5 text-left text-denim-200/40">Reporter</th>
                </tr></thead>
                <tbody>
                  <tr v-if="!failureHistory.length">
                    <td colspan="6" class="py-4 text-center text-denim-200/30">No failure history</td>
                  </tr>
                </tbody>
              </table>
              <div class="mt-3 text-[10px] text-denim-200/30 space-y-0.5 border-t border-denim-700/20 pt-3">
        ...
        <div v-if="showQR" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showQR=false">
          ...
          <div class="relative bg-denim-800 border border-denim-600/40 rounded-2xl shadow-2xl w-full max-w-xs text-center">
            <div class="px-5 py-4 border-b border-denim-700/40 flex items-center justify-between">
              ...
              <button class="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-white/10 text-denim-200/50" @click="showQR=false">✕</button>
            </div>
            <div class="p-6">
              <!-- SVG QR placeholder -->
              ...
              <button class="btn-secondary w-full justify-center mt-4 text-xs">Download QR</button>
            </div>
          </div>
        </div>
      </Transition>
...
const identityRows = computed(() => [
  ...
  {label:"Coordinate (Lat, Long)", value: `${eq.value.lat}, ${eq.value.lng}`},
...
const troubleRecords = ref([
  {id:"TBL-2025-0003", name:"AC Compressor Failure", date:"2025-11-03", operation:"Down", condition:"Freon habis, kompresor lemah", impact:"Ruangan panas, ketidaknyamanan penghuni", reporter:"Ahmad Fauzi", status:"Finished"},
])
